...
1) 大型股「與外資同向」報酬 by 年度、by 產業 (檢驗核心結論是否穩健)
...
def main():
    ...
    # 逐年 12m 為正(外資方向正確)的年數 vs 為負(投信方向正確)
    neg_years = (yr["12m"] < 0).sum()
    print(f"大型股: 16 年中有 {neg_years} 年『與外資同向 12M 報酬為負』(投信方向正確)")

    fig, ax = plt.subplots(figsize=(8, 11))
    sns.heatmap(yr, annot=True, fmt=".1f", center=0, cmap="RdBu_r",
                cbar_kws={"label": "與外資同向報酬 %"}, ax=ax)
    ax.set(title="大型股逐年『與外資同向』報酬 (%)\n(藍=投信方向正確 / 紅=外資方向正確)", xlabel="持有期間", ylabel="")
    ps.save(fig, "rob_largecap_year.png")

    # by 產業 (12m, 樣本 >= 100)
    big["ind"] = short_ind(big["tse_industry"])
    gi = big.groupby("ind")["foll_F_12m"]
    ind_tbl = pd.DataFrame({"n": gi.size(), "與外資同向12m報酬%": gi.mean() * 100})
    ind_tbl = ind_tbl[ind_tbl["n"] >= 100].sort_values("與外資同向12m報酬%")
    save_tab(ind_tbl.reset_index(), "rob_largecap_industry.csv", index=False)

    fig, ax = plt.subplots(figsize=(10, 11))
    colors = ["#2c7fb8" if v < 0 else "#d95f0e" for v in ind_tbl["與外資同向12m報酬%"]]
    ax.barh(ind_tbl.index, ind_tbl["與外資同向12m報酬%"], color=colors)
    ax.axvline(0, color="k", lw=.8)
    ax.set(title="大型股各產業『與外資同向』12M 報酬 (%)\n(藍=投信方向正確 / 橘=外資方向正確)", xlabel="與外資同向報酬 %", ylabel="")
    ...
    for k in PATH_K:
        ...
        cum[k] = opp["foreign_side"].values * r  # 與外資同向
    ...
    ax.set(title="對作後『與外資同向』累積還原報酬 (event-time)",
